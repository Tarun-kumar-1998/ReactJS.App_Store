...
class Appstore extends Component {
  ...
  render() {
    const {selectedTabId, searchApp} = this.state
    const filteredApps = this.filteredApp()
    const searchResults = filteredApps.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(searchApp.toLowerCase()),
    )

    return (
      <div className="bg">
        <h1 className="hd">App Store</h1>
